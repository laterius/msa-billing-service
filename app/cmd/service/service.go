...
func main() {
	...
	//Handlers
	server.POST("/account", api.CreateAccount(newService))
	server.GET("/balance/:userId", api.GetBalanceHandler(newService))
	server.PUT("/balance/:userId", api.ChangeBalanceHandler(newService))
